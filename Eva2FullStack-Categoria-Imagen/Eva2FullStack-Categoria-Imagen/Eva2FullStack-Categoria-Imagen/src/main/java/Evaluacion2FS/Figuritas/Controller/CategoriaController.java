...
public class CategoriaController {
    ...
    public ResponseEntity<CategoriaDTO>BuscarPorId(@PathVariable Integer id){
        try {
            ...
            return new ResponseEntity<>(categoria,HttpStatus.OK);// esta linea que es lo que hace 
            //1- retorna una response entiy(en este caso el metodo de buscar id que es lo primero que declaramos)
            //2-damos aviso al usuario y anosotros de que el metodo funciono correctamente a traves del status OK
        } catch (Exception e) {
    ...
    public ResponseEntity<List<CategoriaDTO>>GetStatusTrue(){
        ...
        if(categorias.isEmpty()){
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);//NO_CONTENT no es lo mismo que NOT_FOUND 
            //No content hace referencia a que si bien se encontro el metodo y se ejecuto este no tenia datos dentro
            //Not Found hace una referencia a que no se a encontrado directamente el metodo al cual se dirijia por lo tanto hay que revisar la cadena
            //model->repository->service->controller (comunmente el error esta entre el service o el controller)
        }
        return new ResponseEntity<>(HttpStatus.OK);
    }
